...
function MapGraphic({ selectedId, onSelect }: { selectedId: string; onSelect: (id: string) => void }) {
  return <div className="relative min-h-[420px] flex-1 overflow-hidden rounded-xl border border-[#c8ddd9] bg-[#eef7f4] sm:min-h-[500px]">
    ...
    <svg viewBox="0 0 100 100" className="h-full w-full p-5" role="img" aria-label="Risk map of Dima Hasao district">
      <defs><filter id="mapShadow"><feDropShadow dx="0" dy="1" stdDeviation="1" floodColor="#5d8c88" floodOpacity=".12" /></filter></defs>
      <path d="M31 8 C39 3, 52 7, 59 12 C68 10, 78 17, 84 25 C88 34, 83 42, 90 51 C87 60, 91 69, 83 77 C73 82, 69 91, 57 90 C48 96, 39 88, 30 91 C20 87, 16 77, 11 70 C13 59, 7 53, 12 44 C10 34, 17 28, 20 20 C24 17, 24 11, 31 8 Z" fill="#d9ebe2" stroke="#75a9a0" strokeWidth="1.4" filter="url(#mapShadow)" />
...
function MapView() {
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_315px]">
      <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-panel sm:p-4"><div className="mb-3 flex items-center justify-between px-1"><p className="text-xs font-bold text-slate-700">Current location risk</p><div className="flex items-center gap-3 text-[10px] text-slate-400"><span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-500" />Low</span><span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-amber-400" />Moderate</span><span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-orange-500" />High</span><span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-red-500" />Severe</span></div></div><MapGraphic selectedId={selectedId} onSelect={setSelectedId} /></div>
